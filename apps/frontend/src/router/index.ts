    ...
    },
    {
      ...
      children: [
        ...
        },
        {
          ...
          component: () =>
            import("../views/auth/PasswordResetConfirmationView.vue"),
        },
        {
          path: "request",
...
router.beforeEach(async (to, from, next) => {
  ...
  if (!to.meta.bypassAuth) {
    if (
      !userStore.accessTokenExpires ||
      userStore.accessTokenExpires < new Date()
    ) {
      await userStore.refreshToken();
    }

    if (!userStore.isAuthenticated) {
      if (to.meta.storeIntentedRoute) {
