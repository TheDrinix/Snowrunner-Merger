...
const handleLinkAccount = async () => {
  try {
    const res = await http.post<LoginResponse>(
      `/auth/${provider.value}/link-account`,
      {
    ...
    createToast(
      ...
      `Your ${provider.value} account has been linked to your account.`,
      "success",
    );
    ...
    if (e instanceof AxiosError) {
      if (e.response?.status === 401) {
        ...
        createToast(
          ...
          `Your ${provider.value} account is already linked to another account. Please login using that account.`,
          "error",
        );
        return router.push({ name: "login" });
        ...
        createToast(
          ...
          e.response.data.message ||
            "Failed to link accounts, please try again later",
          "error",
        );
        return router.push({ name: "login" });
      }
    ...
    createToast(
      ...
      `Failed to link ${provider.value} account, please try again later`,
      "error",
    );
    router.push({ name: "login" });
  }
    ...
    <div class="card bg-base-200 shadow-2xl border border-warning/30">
      <div class="card-body p-8 text-center">
        <div class="flex items-center justify-center gap-4 mb-6">
          <div
            class="w-16 h-16 rounded-full bg-base-300 flex items-center justify-center shadow-inner"
          >
          ...
          <div class="flex flex-col items-center">
            <svg
              ...
              class="h-6 w-6 text-warning animate-pulse"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
          ...
          <div
            class="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center border border-primary/20"
          >
        ...
        <p class="text-sm opacity-80 leading-relaxed mb-6">
          The email
          <span class="badge badge-ghost font-mono text-xs">{{ email }}</span>
          is already registered. Would you like to link your
          <span class="font-bold capitalize">{{ provider }}</span> account to
