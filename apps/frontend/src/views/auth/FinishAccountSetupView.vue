...
if (!route.query.token || !route.query.email) {
  createToast(
    ...
    "Your completion token is missing",
    "error",
  );
...
const handleAccountCompletion = async () => {
  ...
  try {
    const res = await http.post<LoginResponse>(
      `/auth/${provider.value}/finish-account-setup`,
      {
    ...
    createToast(
      ...
      "Your account has been finished",
      "success",
    );
    ...
    if (e instanceof AxiosError && e.response?.status === 401) {
      ...
      createToast(
        ...
        e.response.data.message ||
          "Failed to finish account setup, please try again later",
        "error",
      );
      return router.push({ name: "login" });
    }
    ...
    createToast(
      "Account setup failed",
      `Failed to finish ${provider.value} account setup, please try again later`,
      "error",
    );
    router.push({ name: "login" });
  }
    ...
    <div class="card bg-base-200 shadow-2xl border border-base-300">
      <div class="card-body p-8">
        <div class="text-center mb-8">
          <div
            class="badge badge-primary font-black mb-2 px-4 uppercase tracking-tighter"
          >
            Phase 2: Identity
          </div>
          <h2 class="text-3xl font-black uppercase">Finalize Account</h2>
          <p class="text-sm opacity-60 mt-1">
            We've linked your account. Just a few more details to get your
            engine started.
          </p>
        </div>
        ...
        <form @submit.prevent="handleAccountCompletion" class="space-y-4">
          <div class="form-control">
            <h3 class="label px-1 py-1">
              <span
                class="label-text-alt opacity-50 font-bold uppercase tracking-widest"
                >Verified Email</span
              >
            </h3>
            <div class="relative group">
              <label
                class="transition-all input input-bordered flex items-center gap-2"
              >
                ...
                <input
                  ...
                  v-model="email"
                  type="text"
                  class="w-full bg-base-300 opacity-80 cursor-not-allowed border-dashed"
                  name="email"
                  placeholder="Email"
                />
              </label>
              <div
                class="absolute right-4 top-3 tooltip tooltip-left"
                data-tip="Provided by external service"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-5 w-5 text-success opacity-50"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
          ...
          <div class="space-y-1">
            ...
            >
              <template #icon-prepend
                ><Icon name="person" class="opacity-50"
              /></template>
            </TextInput>
          ...
          <button
            :disabled="loading"
            type="submit"
            class="btn btn-primary btn-block shadow-lg mt-4 h-14"
          >
