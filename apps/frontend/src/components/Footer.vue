<script setup lang="ts">
import Icon from "@/components/icon.vue";

const year = new Date().getFullYear();
</script>

<template>
  <footer class="footer p-10 bg-base-300 text-base-content mt-auto border-t border-base-200">
    <aside>
      <div class="flex items-center gap-2 mb-2">
        <div class="bg-primary text-primary-content p-1 rounded-md">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
          </svg>
        </div>
        <span class="text-lg font-black uppercase tracking-tighter">
        Snowrunner<span class="text-primary">Merger</span>
      </span>
      </div>
      <p class="max-w-xs opacity-60 text-sm">
        Helping truckers keep their progress together since 2024. Not affiliated with Saber Interactive.
      </p>
      <p class="mt-4 text-xs font-bold opacity-40">
        &copy; {{ year }} — All rights reserved.
      </p>
    </aside>

    <nav>
      <h6 class="footer-title opacity-100 text-primary">Development</h6>
      <a href="https://github.com/TheDrinix/snowrunner-merger-api" target="_blank" class="link link-hover flex items-center gap-2">
        <Icon name="code" class="w-4 h-4" />
        API Source
      </a>
      <a href="https://github.com/TheDrinix/snowrunner-merger-web" target="_blank" class="link link-hover flex items-center gap-2">
        <Icon name="terminal" class="w-4 h-4" />
        Web Source
      </a>
    </nav>

    <nav>
      <h6 class="footer-title opacity-100 text-primary">Policies</h6>
      <RouterLink class="link link-hover" to="/privacy">Privacy Policy</RouterLink>
      <RouterLink class="link link-hover" to="/terms-of-use">Terms of Service</RouterLink>
    </nav>

    <nav>
      <h6 class="footer-title opacity-100 text-primary">Community</h6>
      <div class="grid grid-flow-col gap-4">
        <a href="https://github.com/TheDrinix" target="_blank" class="btn btn-ghost btn-sm btn-square">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="fill-current"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/></svg>
        </a>
      </div>
    </nav>
  </footer>
</template>

<style scoped>

</style>